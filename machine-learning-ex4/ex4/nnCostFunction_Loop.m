...
J = sum(J(:)) + lambda/(2*m)*(sum(sum(Theta1_tmp.^2)) + sum(sum(Theta2_tmp.^2)));
...
for t =1:m
    ...
    %3. delta2
    d2 = Theta2_tmp' * d3 .* sigmoidGradient(z2);
...
Theta1_reg = [zeros(size(Theta1_tmp,1),1) Theta1_tmp];
Theta2_reg = [zeros(size(Theta2_tmp,1),1) Theta2_tmp];
